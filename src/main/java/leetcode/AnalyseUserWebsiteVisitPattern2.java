...
public class AnalyseUserWebsiteVisitPattern2 {
    ...
    private Set<String> generatePatterns(List<String> websites) {
        ...
        dfs(websites, 0, new ArrayList<>(), patterns);
        return patterns;
    }

    // Generate all 3-permutations with backtracking
    private void dfs(List<String> websites, int index, List<String> currentWebsites, Set<String> patterns) {
        ...
        for (int i = index; i < websites.size(); i++) {
            ...
            dfs(websites, i + 1, currentWebsites, patterns);
